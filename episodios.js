// episodios.js
const episodios = [
    {
        id: 1,
        titulo: "Ep. 1 - O que acontece se eu não pagar uma dívida?",
        autor: "Luã Gomes e Lucas Soares - 9ºA",
        data: "Março 2026",
        duracao: "3:00 minutos",
        descricao: "Neste episódio, o Luã e o Lucas exploram o que acontece quando uma dívida não é paga, abordando as possíveis consequências como juros de mora, contacto por entidades credoras e até processos judiciais. Ao longo do podcast, explicam de forma simples como evitar estas situações e a importância de uma boa gestão financeira no dia-a-dia.",
        imagem: "images/img_1.jpg",
        audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_1.mp3",
        topicos: [
            "Definição de dívida",
            "Consequências de não pagar dívidas",
            "Dívidas boas vs. dívidas más",
            "Erros jovens em gestão financeira"
        ],
        participantes: ["Luã Gomes e Lucas Soares"],
        infoExtra: "Sabias que... Em Portugal, o incumprimento de crédito pode levar à inclusão do nome do devedor na Central de Responsabilidades de Crédito do Banco de Portugal, o que dificulta o acesso a novos empréstimos. Além disso, os juros de mora podem aumentar significativamente o valor inicial da dívida, tornando-a mais difícil de pagar com o tempo. Em casos mais graves, pode haver penhora de bens ou rendimentos.",
        quiz: [
            {
                pergunta: "O que pode acontecer se não pagares uma dívida dentro do prazo?",
                opcoes: [
                    "A dívida desaparece automaticamente",
                    "São aplicados juros de mora",
                    "Recebes um prémio por atraso",
                    "Nada acontece"
                ],
                resposta: 1
            },
            {
                pergunta: "Quem pode tentar recuperar uma dívida não paga?",
                opcoes: [
                    "Apenas amigos e familiares",
                    "Apenas o banco",
                    "Entidades credoras ou empresas de cobrança",
                    "Ninguém"
                ],
                resposta: 2
            },
            {
                pergunta: "Qual é uma boa forma de evitar dívidas?",
                opcoes: [
                    "Ignorar contas",
                    "Pedir mais empréstimos",
                    "Fazer um orçamento e controlar despesas",
                    "Gastar tudo o que se ganha"
                ],
                resposta: 2
            }
        ]
    },
    {
    id: 2,
    titulo: "Ep. 2 - O que é 'poupar' e o que é 'investir'? É a mesma coisa?",
    autor: "Leandro Batista - 9ºA",
    data: "Março 2026",
    duracao: "01:30 minutos",
    descricao: "Neste episódio do podcast €u Falo de Dinheiro, o Leandro explica a diferença entre poupar e investir. Poupar é guardar dinheiro de forma segura, como numa conta bancária ou num cofre, sem risco de o perder. Investir é usar o dinheiro para comprar ativos como ações, negócios próprios ou criptomoedas, assumindo riscos para tentar obter maiores retornos. O episódio destaca que não são a mesma coisa e que o ideal é usar ambas as estratégias: primeiro criar uma poupança de emergência e depois investir para fazer o dinheiro crescer.",
    imagem: "images/img_2.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_2.mp3",
    topicos: [
        "Definição de poupança",
        "Definição de investimento",
        "Riscos e segurança",
        "Planeamento financeiro"
    ],
    participantes: ["Leandro Batista"],
    infoExtra: "Sabias que... em Portugal, a taxa de poupança das famílias tem vindo a diminuir? Em 2023, rondava os 6% do rendimento disponível, abaixo da média europeia. Investir, mesmo com pequenos montantes, pode ajudar a combater a inflação e a criar riqueza a longo prazo, graças ao poder dos juros compostos. Por exemplo, um investimento de 100€ com uma taxa de retorno de 5% ao ano pode transformar-se em cerca de 163€ passados 10 anos, sem qualquer esforço adicional.",
    quiz: [
        {
            pergunta: "O que significa poupar dinheiro?",
            opcoes: [
                "Guardar dinheiro sem correr risco de o perder",
                "Comprar ações na bolsa de valores",
                "Pedir um empréstimo ao banco",
                "Gastar todo o dinheiro que se recebe"
            ],
            resposta: 0
        },
        {
            pergunta: "Qual é um exemplo de investimento mencionado no episódio?",
            opcoes: [
                "Conta poupança no banco",
                "Criptomoeda",
                "Cofre em casa",
                "Cartão de crédito"
            ],
            resposta: 1
        },
        {
            pergunta: "Segundo o episódio, qual é a estratégia recomendada para gerir o dinheiro?",
            opcoes: [
                "Apenas poupar, nunca investir",
                "Apenas investir, nunca poupar",
                "Primeiro poupar para emergências, depois investir",
                "Gastar tudo e não se preocupar com o futuro"
            ],
            resposta: 2
        }
    ]
},
    {
    id: 3,
    titulo: "Ep. 3 - Para que serve (mesmo) ter uma conta no banco?",
    autor: "Eva Oliveira e Vitória Brioso - 9ºA",
    data: "Março 2026",
    duracao: "03:20 minutos",
    descricao: "Neste episódio do podcast €u Falo de Dinheiro, a Eva e a Vitória explicam para que serve realmente ter uma conta bancária. Abordam as principais vantagens, como segurança, organização e acesso a serviços essenciais, mas também alertam para os possíveis custos, como comissões de manutenção. Destacam a importância de comparar opções e apresentam a Conta de Serviços Mínimos Bancários, uma alternativa em Portugal para quem quer pagar menos. O episódio ajuda a perceber como uma conta pode ser uma ferramenta útil para gerir o dinheiro no dia-a-dia.",
    imagem: "images/img_3.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_3.mp3",
    topicos: [
        "Vantagens de uma conta bancária",
        "Serviços essenciais (débitos diretos, transferências, MB Way)",
        "Custos e comissões bancárias",
        "Conta de Serviços Mínimos Bancários"
    ],
    participantes: ["Eva Oliveira e Vitória Brioso"],
    infoExtra: "Sabias que... a Conta de Serviços Mínimos Bancários (CSMB) foi criada em Portugal para garantir que todos têm acesso a serviços bancários essenciais a um custo reduzido? Em 2024, o custo máximo anual desta conta era de cerca de 4,30€ por mês (incluindo manutenção, cartão de débito e transferências). Além disso, mais de 90% dos bancos em Portugal aderem a esta solução, promovendo a inclusão financeira de quem tem menos recursos.",
    quiz: [
        {
            pergunta: "Qual é uma das principais vantagens de ter uma conta bancária mencionada no episódio?",
            opcoes: [
                "Poder guardar dinheiro em casa sem riscos",
                "Segurança contra roubos e perdas",
                "Evitar pagar qualquer tipo de imposto",
                "Não precisar de cartões para pagamentos"
            ],
            resposta: 1
        },
        {
            pergunta: "O que é a Conta de Serviços Mínimos Bancários?",
            opcoes: [
                "Uma conta que oferece serviços ilimitados sem custos",
                "Uma conta com custos reduzidos para serviços essenciais",
                "Uma conta exclusiva para jovens",
                "Uma conta que não permite transferências"
            ],
            resposta: 1
        },
        {
            pergunta: "Que cuidado se deve ter ao escolher uma conta bancária?",
            opcoes: [
                "Escolher a primeira que aparece sem comparar",
                "Ignorar as comissões de manutenção",
                "Comparar opções e verificar os custos associados",
                "Abrir várias contas ao mesmo tempo"
            ],
            resposta: 2
        }
    ]
},
    {
    id: 4,
    titulo: "Ep. 4 - Quero comprar uma mota daqui a 2 anos. Como faço um plano?",
    autor: "Lara Abreu e Vitória Brioso - 9ºA",
    data: "Março 2026",
    duracao: "01:05 minutos",
    descricao: "Neste episódio do podcast €u Falo de Dinheiro, a Lara e a Vitória ensinam como criar um plano para comprar uma mota dentro de dois anos. Explicam a importância de definir um objetivo claro, somando todos os custos envolvidos (mota, equipamento, seguros e despesas), e depois dividir esse valor por 24 meses para saber quanto poupar por mês. Também abordam a necessidade de rever despesas, a possibilidade de ajustar o prazo ou escolher uma mota mais barata, e dão a dica de ouro: fazer trabalhos extras para acelerar a meta.",
    imagem: "images/img_4.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_4.mp3",
    topicos: [
        "Definição de metas financeiras",
        "Cálculo de poupança mensal",
        "Análise e corte de despesas",
        "Estratégias para acelerar a poupança"
    ],
    participantes: ["Lara Abreu e Vitória Brioso"],
    infoExtra: "Sabias que... ao comprar uma mota, deves considerar não só o preço do veículo, mas também equipamento de segurança (capacete, luvas, casaco), seguro obrigatório, inspeção e combustível? Estes custos podem representar até 20% do valor inicial da mota. Além disso, poupar com antecedência permite aproveitar promoções ou até negociar um desconto no pagamento a pronto. Em Portugal, o seguro de mota é obrigatório e pode variar consoante a cilindrada, idade do condutor e histórico de sinistros.",
    quiz: [
        {
            pergunta: "Qual é o primeiro passo para comprar uma mota daqui a dois anos, segundo o episódio?",
            opcoes: [
                "Pedir um empréstimo ao banco",
                "Definir o objetivo e somar todos os custos",
                "Comprar a mota imediatamente e pagar depois",
                "Guardar dinheiro sem qualquer planeamento"
            ],
            resposta: 1
        },
        {
            pergunta: "Se não conseguires poupar o valor necessário por mês, o que podes fazer?",
            opcoes: [
                "Desistir do objetivo",
                "Pedir dinheiro emprestado a amigos",
                "Estender o prazo ou escolher uma mota mais barata",
                "Ignorar as despesas e poupar o mesmo"
            ],
            resposta: 2
        },
        {
            pergunta: "Qual é a 'dica de ouro' do episódio para atingir a meta mais rápido?",
            opcoes: [
                "Reduzir todas as despesas de lazer",
                "Fazer trabalhos extras",
                "Apostar em investimentos de risco",
                "Esperar por um salário maior"
            ],
            resposta: 1
        }
    ]
},
    {
    id: 5,
    titulo: "Ep. 5 - Juro Fixo ou Juro Variável?",
    autor: "Bianca Candeias - 9ºA",
    data: "Março 2026",
    duracao: "01:28 minutos",
    descricao: "Neste episódio do podcast €u Falo de Dinheiro, a Bianca explica a diferença entre juro fixo e juro variável em empréstimos, como o crédito à habitação. O juro fixo garante prestações estáveis ao longo do tempo, oferecendo segurança e previsibilidade. Já o juro variável depende da Euribor, que sobe e desce conforme a economia, podendo tornar as prestações mais baratas no início, mas com risco de aumentarem se as taxas subirem. A aluna destaca a importância de compreender estes conceitos antes de pedir um empréstimo e dá uma dica sobre como as decisões do Banco Central Europeu influenciam a Euribor.",
    imagem: "images/img_5.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_5.mp3",
    topicos: [
        "Juro fixo vs. juro variável",
        "Crédito à habitação",
        "Euribor e política monetária",
        "Gestão de risco em empréstimos"
    ],
    participantes: ["Bianca Candeias"],
    infoExtra: "Sabias que... a Euribor (Euro Interbank Offered Rate) é a taxa de juro média a que os bancos europeus emprestam dinheiro entre si? Em Portugal, a maioria dos créditos à habitação com juro variável está indexada à Euribor a 3, 6 ou 12 meses. Em 2022 e 2023, com a subida das taxas de juro pelo Banco Central Europeu para controlar a inflação, muitas famílias viram as suas prestações aumentar significativamente. Por exemplo, um empréstimo de 150.000€ a 30 anos pode ter uma prestação que sobe várias centenas de euros por mês quando a Euribor aumenta 2% ou 3%.",
    quiz: [
        {
            pergunta: "Qual é a principal característica do juro fixo num empréstimo?",
            opcoes: [
                "A prestação pode variar todos os meses",
                "A prestação mantém-se estável ao longo do tempo",
                "Depende da Euribor",
                "É sempre mais barato que o juro variável"
            ],
            resposta: 1
        },
        {
            pergunta: "O que pode fazer a prestação de um empréstimo com juro variável aumentar?",
            opcoes: [
                "A descida da Euribor",
                "A subida da Euribor",
                "A estabilidade da economia",
                "A diminuição da inflação"
            ],
            resposta: 1
        },
        {
            pergunta: "Segundo o episódio, qual é uma vantagem do juro variável no início do empréstimo?",
            opcoes: [
                "A prestação costuma ser mais baixa",
                "A prestação é garantida para sempre",
                "Não há risco de aumentos",
                "É independente do mercado"
            ],
            resposta: 0
        }
    ]
},
    {
    id: 6,
    titulo: "Ep. 6 - Ouvi falar em Ações e Obrigações. O que é isso?",
    autor: "Inês Pereira - 9ºA",
    data: "Março 2026",
    duracao: "02:10 minutos",
    descricao: "Neste episódio do podcast €u Falo de Dinheiro, a Inês descomplica a diferença entre ações e obrigações, dois conceitos fundamentais no mundo dos investimentos. Explica que comprar ações é como comprar um pedacinho de uma empresa, tornando-te sócio e correndo mais risco, mas com potencial de maior retorno. Já as obrigações funcionam como um empréstimo à empresa, onde és credor e recebes juros fixos, sendo geralmente consideradas mais seguras. A Inês usa exemplos práticos, como comprar ações ou obrigações do Benfica, e termina com a dica de ouro da diversificação: não colocar todos os ovos no mesmo cesto.",
    imagem: "images/img_6.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_6.mp3",
    topicos: [
        "Ações vs. Obrigações",
        "Ser sócio vs. ser credor",
        "Risco e retorno",
        "Diversificação de investimentos"
    ],
    participantes: ["Inês Pereira"],
    infoExtra: "Sabias que... em Portugal, o mercado de ações e obrigações é regulado pela CMVM (Comissão do Mercado de Valores Mobiliários)? Historicamente, as ações tendem a oferecer retornos mais elevados a longo prazo, mas com maior volatilidade. Já as obrigações são preferidas por investidores que procuram rendimentos mais previsíveis. A diversificação, recomendada no episódio, é uma estratégia que reduz o risco global da carteira: se as ações caírem, as obrigações podem estabilizar ou valorizar, e vice-versa. É por isso que muitos especialistas sugerem uma combinação dos dois ativos.",
    quiz: [
        {
            pergunta: "O que significa comprar uma ação de uma empresa?",
            opcoes: [
                "Emprestar dinheiro à empresa",
                "Comprar um pedacinho da empresa e tornar-se sócio",
                "Abrir uma conta poupança na empresa",
                "Garantir um salário fixo da empresa"
            ],
            resposta: 1
        },
        {
            pergunta: "Qual é a principal característica das obrigações?",
            opcoes: [
                "São mais arriscadas que as ações",
                "Quem compra torna-se dono da empresa",
                "Quem compra está a emprestar dinheiro e recebe juros",
                "O retorno é sempre negativo"
            ],
            resposta: 2
        },
        {
            pergunta: "Qual é a 'dica de ouro' do episódio para quem está a começar a investir?",
            opcoes: [
                "Investir todo o dinheiro em ações",
                "Investir todo o dinheiro em obrigações",
                "Diversificar, não colocando todos os ovos no mesmo cesto",
                "Guardar dinheiro debaixo do colchão"
            ],
            resposta: 2
        }
    ]
},
{
    id: 7,
    titulo: "Ep. 7 - Porque compramos o que não precisamos?",
    autor: "Alexei Gangan e Alexander Gamito - 9ºA",
    data: "Março 2026",
    duracao: "2:47 minutos",
    descricao: "Neste episódio, o Alexei e o Alexander exploram as razões por trás das compras por impulso. Analisam como as emoções (tristeza, stress ou até felicidade), as promoções agressivas e a influência das redes sociais nos levam a adquirir produtos de que não precisamos. Depois, partilham estratégias práticas para controlar estes impulsos, como a regra das 24 horas, fazer uma lista de compras e definir limites de gasto, ajudando os ouvintes a desenvolver hábitos financeiros mais conscientes e evitar dificuldades financeiras.",
    imagem: "images/img_7.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_7.mp3",
    topicos: [
        "O que são compras por impulso",
        "Emoções e comportamento de compra",
        "Influência das promoções e FOMO",
        "Impacto das redes sociais nas decisões de compra",
        "Estratégias para evitar compras desnecessárias"
    ],
    participantes: ["Alexei Gangan e Alexander Gamito"],
    infoExtra: "Sabias que... Estudos mostram que esperar 24 horas antes de fazer uma compra não planeada pode reduzir significativamente as compras por impulso. Além disso, fazer uma lista de compras e definir um orçamento prévio ajuda a manter o foco nas necessidades reais, evitando gastos desnecessários que podem comprometer o equilíbrio financeiro mensal.",
    quiz: [
        {
            pergunta: "Qual é uma das principais razões que nos levam a fazer compras por impulso?",
            opcoes: [
                "Planeamento financeiro rigoroso",
                "Emoções como tristeza ou stress",
                "Ter uma lista de compras definida",
                "Esperar 24 horas antes de comprar"
            ],
            resposta: 1
        },
        {
            pergunta: "O que significa FOMO no contexto das compras?",
            opcoes: [
                "Fear Of Missing Out - medo de perder uma oportunidade",
                "Financial Organization and Management Option",
                "First Offer Maximum Offer",
                "Free Online Shopping Method"
            ],
            resposta: 0
        },
        {
            pergunta: "Qual é a 'dica de ouro' sugerida para evitar compras por impulso?",
            opcoes: [
                "Comprar imediatamente quando vês uma promoção",
                "Seguir todas as tendências das redes sociais",
                "Esperar pelo menos 24 horas antes de comprar",
                "Ignorar completamente o orçamento"
            ],
            resposta: 2
        }
    ]
},
{
    id: 8,
    titulo: "Ep. 8 - O que é o Banco de Portugal?",
    autor: "Lucas Alambre - 9ºA",
    data: "Março 2026",
    duracao: "1:30 minutos",
    descricao: "Neste episódio, o Lucas explica o papel do Banco de Portugal e do Banco Central Europeu no sistema financeiro. Uma explicação clara e acessível sobre como estas instituições trabalham em conjunto para garantir a estabilidade económica.",
    imagem: "images/img_8.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_8.mp3",
    topicos: [
        "O que é o Banco de Portugal",
        "Funções do Banco Central Europeu",
        "Diferenças entre Banco de Portugal e BCE",
        "Controlo da inflação e taxas de juro",
        "Supervisão do sistema financeiro português"
    ],
    participantes: ["Lucas Alambre"],
    infoExtra: "Sabias que... O Banco de Portugal foi fundado em 1846 e integra o Sistema Europeu de Bancos Centrais desde 1998. Enquanto o Banco Central Europeu, sediado em Frankfurt, define as taxas de juro para toda a zona Euro, o Banco de Portugal supervisiona as instituições financeiras nacionais e protege os direitos dos consumidores financeiros em Portugal.",
    quiz: [
        {
            pergunta: "Qual é a principal função do Banco Central Europeu?",
            opcoes: [
                "Supervisionar apenas os bancos portugueses",
                "Manter a estabilidade dos preços e controlar a inflação",
                "Gerir contas bancárias pessoais",
                "Definir impostos nacionais"
            ],
            resposta: 1
        },
        {
            pergunta: "O que faz o Banco de Portugal?",
            opcoes: [
                "Define a política monetária para toda a Europa",
                "Aplica as decisões do BCE em Portugal e supervisiona bancos nacionais",
                "Emite moeda de outros países",
                "Controla apenas o comércio internacional"
            ],
            resposta: 1
        },
        {
            pergunta: "Qual é a principal diferença entre o Banco de Portugal e o Banco Central Europeu?",
            opcoes: [
                "Não há diferença, são o mesmo banco",
                "O BCE decide para a zona Euro, o Banco de Portugal aplica a nível nacional",
                "O Banco de Portugal é mais importante que o BCE",
                "O BCE só trabalha com bancos privados"
            ],
            resposta: 1
        }
    ]
},
{
    id: 9,
    titulo: "Ep. 9 - Perdi ou ganhei dinheiro com a inflação?",
    autor: "Shubham Silwal e Tomás Jesus - 9ºA",
    data: "Março 2026",
    duracao: "2:40 minutos",
    descricao: "Neste episódio, o Shubham e o Tomás explicam a diferença crucial entre juros nominais e juros reais. Usando um exemplo prático com pizzas, demonstram como a inflação pode reduzir o poder de compra mesmo quando o saldo bancário aumenta. Se o banco paga 2% de juros mas a inflação é de 3%, na realidade estás a perder 1% do teu poder de compra. Os apresentadores partilham a 'dica de ouro': comparar sempre a taxa de juros com a taxa de inflação para saber se o teu dinheiro está realmente a crescer ou a perder valor.",
    imagem: "images/img_9.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_9.mp3",
    topicos: [
        "Diferença entre juros nominais e juros reais",
        "Como a inflação afeta o poder de compra",
        "Exemplo prático: 100€ e o preço das pizzas",
        "Como calcular se estás a ganhar ou perder dinheiro",
        "Estratégias para proteger as poupanças da inflação"
    ],
    participantes: ["Shubham Silwal e Tomás Jesus"],
    infoExtra: "Sabias que... A taxa de juro real é calculada subtraindo a taxa de inflação à taxa de juro nominal. Se o teu depósito rende 2% ao ano mas a inflação é de 3%, a tua taxa real é de -1%. Isto significa que, apesar de teres mais euros na conta, consegues comprar menos bens e serviços do que no ano anterior.",
    quiz: [
        {
            pergunta: "Se o banco paga 2% de juros e a inflação é de 3%, o que acontece ao teu poder de compra?",
            opcoes: [
                "Aumenta 5%",
                "Mantém-se igual",
                "Diminui 1%",
                "Aumenta 1%"
            ],
            resposta: 2
        },
        {
            pergunta: "O que representa o 'valor real' do dinheiro?",
            opcoes: [
                "O número exato que aparece no extrato bancário",
                "A quantidade de bens e serviços que o dinheiro consegue comprar",
                "O valor das moedas e notas em euros",
                "O montante total de juros recebidos"
            ],
            resposta: 1
        },
        {
            pergunta: "Qual é a 'dica de ouro' para avaliar se uma poupança vale a pena?",
            opcoes: [
                "Olhar apenas para a taxa de juro oferecida pelo banco",
                "Comparar a taxa de juro com a taxa de inflação",
                "Guardar o dinheiro em casa para não perder valor",
                "Investir tudo em ações sem pesquisar"
            ],
            resposta: 1
        }
    ]
},
{
    id: 10,
    titulo: "Ep. 10 - Para que serve ter uma conta no Banco?",
    autor: "Vitória Brioso, Lara Abreu e Eva Oliveira - 9ºA",
    data: "Março 2026",
    duracao: "3:20 minutos",
    descricao: "Neste episódio, a Vitória, a Lara e a Eva explicam a importância de ter uma conta bancária no dia-a-dia. Abordam as vantagens como segurança contra roubos, organização financeira através de extratos e facilidade nos pagamentos e recebimentos.",
    imagem: "images/img_10.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_10.mp3",
    topicos: [
        "Vantagens de ter uma conta bancária",
        "Segurança do dinheiro no banco vs. em casa",
        "Organização financeira e extratos",
        "Serviços essenciais (MB Way, transferências)",
        "Conta de Serviços Mínimos Bancários"
    ],
    participantes: ["Vitória Brioso, Lara Abreu e Eva Oliveira"],
    infoExtra: "Sabias que... Em Portugal, a Conta de Serviços Mínimos Bancários é um direito de todos os clientes e inclui serviços como a emissão de cartão de débito, realização de levantamentos e pagamentos de serviços essenciais a um custo mensal máximo definido por lei, promovendo a inclusão financeira.",
    quiz: [
        {
            pergunta: "Qual é uma das principais vantagens de ter dinheiro no banco em vez de em casa?",
            opcoes: [
                "O dinheiro rende mais automaticamente",
                "Está protegido contra roubos, perdas ou danos",
                "Podes gastar sem limite",
                "O banco paga-te para guardar o dinheiro"
            ],
            resposta: 1
        },
        {
            pergunta: "O que é a Conta de Serviços Mínimos Bancários?",
            opcoes: [
                "Uma conta apenas para milionários",
                "Uma conta que garante acesso a serviços essenciais a custo reduzido",
                "Uma conta sem cartão de débito",
                "Uma conta apenas para empresas"
            ],
            resposta: 1
        },
        {
            pergunta: "Qual é a 'dica de ouro' antes de abrir uma conta bancária?",
            opcoes: [
                "Escolher o banco com o edifício mais bonito",
                "Verificar as comissões mensais e serviços incluídos",
                "Abrir conta em todos os bancos",
                "Ignorar os custos de manutenção"
            ],
            resposta: 1
        }
    ]
},
{
    id: 11,
    titulo: "Ep. 11 - O que é a inflação?",
    autor: "Érica Barreto e Vladimir Jones - 9ºB",
    data: "Março 2026",
    duracao: "2:05 minutos",
    descricao: "Neste episódio, a Érica e o Vladimir explicam de forma simples o conceito de inflação. Abordam por que razão os preços sobem com o passar do tempo e como isso afeta o poder de compra das famílias. Uma explicação clara e acessível sobre a razão do dinheiro valer menos quando a inflação sobe.",
    imagem: "images/img_11.jpg",
    audio: "https://pub-c876b1902a0a426d9d354f25c6e000fa.r2.dev/episodio_11.mp3",
    topicos: [
        "O que é a inflação",
        "Por que os preços sobem com o tempo",
        "Diferença entre inflação e aumento de preço isolado",
        "Causas da inflação (procura, custos, expectativas)",
        "Impacto da inflação no poder de compra"
    ],
    participantes: ["Érica Barreto e Vladimir Jones"],
    infoExtra: "Sabias que... A inflação não é apenas um produto que fica mais caro, mas sim o aumento generalizado de preços de muitos bens e serviços ao mesmo tempo (comida, transporte, roupas, energia). Quando há muita procura e poucos produtos disponíveis, ou quando fica mais caro produzir e transportar mercadorias, os preços tendem a subir, reduzindo o poder de compra do dinheiro.",
    quiz: [
        {
            pergunta: "O que é a inflação?",
            opcoes: [
                "Quando apenas um produto fica mais caro",
                "Quando muitos preços sobem ao mesmo tempo com o passar do tempo",
                "Quando os salários aumentam",
                "Quando o banco fecha"
            ],
            resposta: 1
        },
        {
            pergunta: "Qual é uma das causas da inflação?",
            opcoes: [
                "Muita gente quer comprar e não há produtos suficientes",
                "Todos os produtos estão em promoção",
                "O dinheiro vale mais",
                "Os bancos fecham"
            ],
            resposta: 0
        },
        {
            pergunta: "O que acontece ao poder de compra quando há inflação?",
            opcoes: [
                "O dinheiro compra mais coisas",
                "O dinheiro compra a mesma quantidade de coisas",
                "O dinheiro compra menos coisas",
                "O dinheiro desaparece"
            ],
            resposta: 2
        }
    ]
}
];